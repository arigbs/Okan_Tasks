...
#var MainControl := preload("res://src/MainControl.tscn").instance()
...
func check_position():
	...
	#popup_container.set_position(Vector2(popup_container.get_position().x - difference_x, popup_container.get_position().y - difference_y))
	popup_container.set_position(Vector2(-600, -100))
#	popup_container.set_as_toplevel(true)



#	var cal_Viewport = VisualServer.viewport_create()
#
#	var canvas_rid = get_canvas_item()
#	var canvasLayer_rid = get_canvas() 
#	# You may need to adjust these values
#	VisualServer.canvas_item_set_draw_index(canvas_rid, 100)
#	VisualServer.canvas_item_set_z_index(canvas_rid, 100)
	#VisualServer.canvas_item_set_parent(canvas_rid, canvasLayer_rid)

	#popup_container.set_position(Vector2(OS.get_window_size().x / 2, OS.get_window_size().y / 2))
	#popup_container.set_position(Vector2(ProjectSettings.get_setting("display/window/size/width") / 4, ProjectSettings.get_setting("display/window/size/height") / 4))
	#print(str(popup_container.position))
	#set_z_index(10)
...
func _on_dueDateButton_pressed():
	print("due date button pressed")
